...
def configSimulation(name, anecroic=True):
    ...
    if name == 'wall right':
        S = config_wall_right(S)
    
    elif name == 'wall left':
        S = config_wall_left(S)
    
    elif name == 'wall above':
        S = config_wall_above(S)
    
    elif name == 'wall below':
        S = config_wall_below(S)
...
def config_wall_right(S):
    # walls
    p = {}
    p['upper_left_x'] = 0.75
    p['upper_left_y'] = 0.25
    p['lower_right_x'] = 0.8
    p['lower_right_y'] = 0.75
    p['value'] = 800
    ...
    sources_parameter['frequency'] = 60
...
def config_wall_left(S):
    # walls
    p = {}
    p['upper_left_x'] = 0.2
    p['upper_left_y'] = 0.25
    p['lower_right_x'] = 0.25
    p['lower_right_y'] = 0.75
    p['value'] = 800
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)
...
def config_room_with_lens(S):
    ...
    # --- Right wall with a hole ---
    # Dividimos em duas partes para deixar o vão no meio
    ...
    p = {'upper_left_x': 0.75,
         ...
         'lower_right_x': 0.8,
         'lower_right_y': 0.42,
         'value': 800} # Vai até 0.42
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # right wall  - lower part
    p = {'upper_left_x': 0.75,
         'upper_left_y': 0.58, # Começa em 0.58
         'lower_right_x': 0.8,
         'lower_right_y': 0.75, 
         'value': 800}
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)
    ...
    # lens (permissiveness) ---
    # inserted at the hole (y from 0.42 to 0.58)
    
    lens_p = {}
    lens_p['cx'] = 0.775    # Centralizado na espessura da parede (entre 0.75 e 0.80)
    lens_p['cy'] = 0.5      # Centralizado na altura
    lens_p['r_left'] = 0.15 # Curvatura esquerda
    lens_p['r_right'] = 0.15 # Curvatura direita
    lens_p['R'] = 0.08      # Raio da abertura (0.08 + 0.08 = 0.16, tamanho do buraco)
    lens_p['er'] = 2.0      # Índice de refração (densidade óptica) da lente
    lens_p['smooth'] = 200  # Suavização das bordas
    
    # Gera a lente na matriz de permissividade (er)
    S['er'] = generateEpsilon(S['Nx'], S['Ny'], 'lens', lens_p)
    
    # --- source inside the walls ---
    sources_parameter = {}
    sources_parameter['cx'] = 0.5       # Centro da sala em X
    sources_parameter['cy'] = 0.5       # Centro da sala em Y
    sources_parameter['amplitude'] = 1000
...
def config_room_with_receiver(S):

    # wall BELOW (Maintained)
    p = {}
    p['upper_left_x'] = 0.2
    p['upper_left_y'] = 0.75
    p['lower_right_x'] = 0.8
    ...
    # 3. Parede de CIMA (ALTERADA)
    p = {}
    p['upper_left_x'] = 0.2
    p['upper_left_y'] = 0.20   # Define o topo
    p['lower_right_x'] = 0.8
    p['lower_right_y'] = 0.25
    p['value'] = 800
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # Parte Superior da Parede Direita
    p = {'upper_left_x': 0.75,
         'upper_left_y': 0.25, 
         'lower_right_x': 0.8,
         'lower_right_y': 0.42,
    ...
    # Parte Inferior da Parede Direita
    p = {'upper_left_x': 0.75,
         'upper_left_y': 0.58, 
         'lower_right_x': 0.8,
         'lower_right_y': 0.75, 
    ...
    # --- RECEPTOR (ADICIONADO) ---
    receiver_parameter = {}
    receiver_parameter['cx'] = 0.7  # Fora da sala
    receiver_parameter['cy'] = 0.5  # Alinhado com a lente
    
    if 'receivers' not in S:
        S['receivers'] = []
    S['receivers'].append(receiver_parameter)

    return S
 
def config_flat_room_with_receiver(S):
...
def config_room_with_lens(S):
    ...
    p = {'upper_left_x': 0.2, 'upper_left_y': 0.25, 
         'lower_right_x': 0.25, 'lower_right_y': 0.75, 'value': 800}
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # Parede de Baixo (Completa)
    p = {'upper_left_x': 0.2, 'upper_left_y': 0.75, 
         'lower_right_x': 0.8, 'lower_right_y': 0.8, 'value': 800}
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # Parede de Cima (Completa)
    p = {'upper_left_x': 0.2, 'upper_left_y': 0.20, 
         'lower_right_x': 0.8, 'lower_right_y': 0.25, 'value': 800}
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # --- PAREDE DIREITA (COM O BURACO) ---
    # Dividimos em duas partes para deixar o vão no meio

    # Parte Superior da Parede Direita
    p = {'upper_left_x': 0.75, 'upper_left_y': 0.25, 
         'lower_right_x': 0.8, 'lower_right_y': 0.42, 'value': 800} # Vai até 0.42
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)

    # Parte Inferior da Parede Direita
    p = {'upper_left_x': 0.75, 'upper_left_y': 0.58, # Começa em 0.58
         'lower_right_x': 0.8, 'lower_right_y': 0.75, 'value': 800}
    S['s'] += generateConductivity(S['Nx'], S['Ny'], 'box', p)


    # --- 2. A LENTE (Permissividade) ---
    # Inserida no buraco (y de 0.42 a 0.58)
